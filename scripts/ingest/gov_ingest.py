"""Government Document Ingestor - Handles bulk data from govinfo.gov
and congress.gov"""
...
class GovernmentDataIngestor:
    ...
    def _connect_vectorstore(self):
        """Initialize and connect to vector store with retries and cleanup."""
        max_retries = 3
        for attempt in range(max_retries):
            try:
                ...
                self.logger.info(
                    "Connected to vector store successfully (attempt %d)",
                    attempt + 1,
                )
                return
                ...
                if hasattr(self.vector_store, 'disconnect'):
                    try:
                        ...
                        self.logger.error(
                            "Error disconnecting from vector store: %s",
                            e,
                            exc_info=True,
                        )
    ...
    def process_and_store(self, documents: List[Dict], collection: str = "government_docs") -> None:
        ...
        for doc in documents:
            try:
                ...
                error_count += 1
                self.logger.error(
                    f"Error processing document {doc.get('packageId')}: {str(e)}",
                    exc_info=True,
                )
        ...
        try:
            if processed:
                ...
                self.logger.info(
                    f"Successfully stored {len(processed)} documents in {collection}"
                )
                if error_count > 0:
                    self.logger.warning(
                        f"Skipped {error_count} documents due to processing errors"
                    )
            else:
